...
def initialize(args):
    ...
    audio_dir = "data/audio/hot_bod/"

    audio, sr = rosa.load(audio_dir + "KICK.wav", offset=args.offset, duration=args.duration)
    args.kick_onsets = ar.onsets(audio, sr, args.n_frames, margin=1, smooth=2, power=1,clip=95)
    
    audio, sr = rosa.load(audio_dir + "SNARE_BOTTOM.wav", offset=args.offset, duration=args.duration)
    args.snare_onsets = ar.onsets(audio, sr, args.n_frames, margin=1, smooth=1, power=1, clip=95)
    ...
    audio, sr = rosa.load(audio_dir + "E_BASS_DI.wav", offset=args.offset, duration=args.duration)
    args.bass_onsets = ar.onsets(audio, sr, args.n_frames, margin=8, smooth=1, power=1, fmin=30, clip=92)
    
    #audio, sr = rosa.load(audio_dir + "NORD.R.wav", offset=args.offset, duration=args.duration)
    #nord_onsets = ar.onsets(audio, sr, args.n_frames, smooth=2, clip=99, power=1, fmin=500)
    nord_onsets = th.zeros_like(args.bass_onsets)

    audio, sr = rosa.load(audio_dir + "PROPHET.wav", offset=args.offset, duration=args.duration)
    prophet_onsets = ar.onsets(audio, sr, args.n_frames, smooth=1, clip=92, power=1, fmin=500)

    #audio, sr = rosa.load(audio_dir + "PNO.wav", offset=args.offset, duration=args.duration)
    #piano_onsets = ar.onsets(audio, sr, args.n_frames, smooth=1, clip=99, power=1, margin=4, fmin=500)
    piano_onsets = th.zeros_like(args.bass_onsets)

    args.piano_onsets = th.fmax(th.fmax(nord_onsets, prophet_onsets), piano_onsets)
    
    print("done")

    #print("onsets:")
    ar.plot_signals([args.kick_onsets, args.snare_onsets, drum_onsets, args.drum_onsets, args.bass_onsets, 
                     nord_onsets, prophet_onsets, piano_onsets, args.piano_onsets])
    
    return args


def bass_solo_animal_bass_latents(selection, args):
    latents = ar.spline_loops(selection, args.n_frames, 1, loop=False)
    #latents = ar.gaussian_filter(latents.float(), 3, causal=0.1)
    return latents

def bass_solo_trusses_latents(selection, args):
    latents = ar.slerp_loops(selection, args.n_frames, 1, loop=False)
...
def synth_solo_animals_latents(selection, args):
    latents = ar.spline_loops(selection, args.n_frames, 1, loop=False)
...
def get_latents(selection, args):
    return bass_solo_animal_bass_latents(selection, args)
...
def bass_solo_animal_bass_noise(height, width, scale, num_scales, args):
...
def bass_solo_trusses_noise(height, width, scale, num_scales, args):
    if width > 512:  # larger sizes don't fit in VRAM, just use default or randomize
        return None
...
def synth_solo_animals_noise(height, width, scale, num_scales, args):
    if width > 512:  # larger sizes don't fit in VRAM, just use default or randomize
        return None
...
def drum_animals_noise(height, width, scale, num_scales, args):
    if width > 512:  # larger sizes don't fit in VRAM, just use default or randomize
        return None

    drum_onsets = args.drum_onsets[:, None, None, None].cuda()
    kick_onsets = args.kick_onsets[:, None, None, None].cuda()
    snare_onsets = args.snare_onsets[:, None, None, None].cuda()
    ...
    if width > 16 and width < 128:
        a = 0.75
        noise = a * kick_onsets * noise_noisy + (1 - a * kick_onsets) * noise
    if width > 32:
        a = 0.75
        noise = a * drum_onsets * noise_noisy + (1 - a * drum_onsets) * noise
...
def drum_trip_noise(height, width, scale, num_scales, args):
    if width > 512:  # larger sizes don't fit in VRAM, just use default or randomize
        return None
...
def synth_trip_noise(height, width, scale, num_scales, args):
    if width > 512:  # larger sizes don't fit in VRAM, just use default or randomize
        return None

    synth_onsets = args.piano_onsets[:, None, None, None].cuda()
    drum_onsets = args.drum_onsets[:, None, None, None].cuda()
    ...
    if width < 64:
        a = 1
        noise = a * synth_onsets * noise_noisy + (1 - a * synth_onsets) * noise
    if width > 32:
        a = 0.75
        noise = a * drum_onsets * noise_noisy + (1 - a * drum_onsets) * noise
...
def bass_trip_noise(height, width, scale, num_scales, args):
    if width > 512:  # larger sizes don't fit in VRAM, just use default or randomize
        return None
...
def get_noise(height, width, scale, num_scales, args):
    return bass_solo_trusses_noise(height, width, scale, num_scales, args)
